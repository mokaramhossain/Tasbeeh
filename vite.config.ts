  ...
  plugins: [
    ...
    VitePWA({
      ...
      manifest: {
        ...
        short_name: 'Tasbeeh',
        description:
          "Build a consistent daily dhikr habit: the after-salah routine, Asma ul Husna with the verse each name is found in, and du'as for every moment. Private, offline and free.",
        theme_color: '#0B1410',
        ...
        // Shown in the install sheet on Android, which then reads like a store
        // listing rather than a bare prompt. Generated by `npm run shots`.
        screenshots: [
          { src: 'screenshots/1-home.png', label: 'Home: the routine and the names, one tap each' },
          { src: 'screenshots/2-reader.png', label: 'Each name with the verse it is found in' },
          { src: 'screenshots/4-names.png', label: 'The ninety-nine names, read through' }
        ].map((shot) => ({ ...shot, sizes: '1080x1920', type: 'image/png', form_factor: 'narrow' })),
        icons: [
          {
            src: 'icon-192.png',
            sizes: '192x192',
      ...
      workbox: {
        ...
        // The install-sheet screenshots are the same: read by the browser when
        // offering to install, never by the running app.
        globIgnores: ['**/share-card.png', '**/screenshots/**'],
